...
class Solution2699 : public ::testing::Test {
public:
    static vector<vector<int>>
    modifiedGraphEdges(int n, vector<vector<int>> &edges, int source, int destination, int target) {
...
private:
    static void runDijkstra(const vector<vector<pair<int, int>>> &adjacencyList, vector<vector<int>> &edges,
                            vector<vector<int>> &distances, int source, int difference, int run) {
...
TEST(Solution2699, T1) {
    int n = 5;
    vector<vector<int>> edges{{4, 1, -1},
                              {2, 0, -1},
                              {0, 3, -1},
                              {4, 3, -1}};
    int source = 0;
    int destination = 1;
    int target = 5;
    vector<vector<int>> expected{{4, 1, 1},
                                 {2, 0, 3},
                                 {0, 3, 3},
                                 {4, 3, 1}};
    ASSERT_TRUE(expected == Solution2699::modifiedGraphEdges(n, edges, source, destination, target));
}

TEST(Solution2699, T2) {
    int n = 3;
...
TEST(Solution2699, T3) {
    int n = 4;
    vector<vector<int>> edges{{1, 0, 4},
                              {1, 2, 3},
                              {2, 3, 5},
                              {0, 3, -1}};
    int source = 0;
    int destination = 2;
    int target = 6;
    vector<vector<int>> expected{{1, 0, 4},
                                 {1, 2, 3},
                                 {2, 3, 5},
                                 {0, 3, 1}};
    ASSERT_TRUE(expected == Solution2699::modifiedGraphEdges(n, edges, source, destination, target));
