...
class Solution2678 : public ::testing::Test {
public:
    static int countSeniors(vector<string> &details) {
        int cnt = 0;
        for (auto &s: details) {
            if (s.substr(11, 2) > "60") cnt++;
        }
        return cnt;
    }
};

TEST(Solution2678, T1) {
    vector<string> details{"7868190130M7522", "5303914400F9211", "9273338290F4010"};
    int expected = 2;
    EXPECT_TRUE(expected == Solution2678::countSeniors(details));
}

TEST(Solution2678, T2) {
